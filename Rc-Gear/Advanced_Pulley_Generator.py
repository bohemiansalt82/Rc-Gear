...
def create_advanced_pulley(params):
    try:
        ...
        # 메인 치성부
        ext_body = extrudes.addByDistanceExtent(prof, adsk.core.ValueInput.createByReal(mm_to_cm(width)), adsk.fusion.FeatureOperations.NewBodyFeatureOperation)
        ...
        extrudes.addByDistanceExtent(sk_f1.profiles.item(0), adsk.core.ValueInput.createByReal(-mm_to_cm(f_t)), adsk.fusion.FeatureOperations.JoinFeatureOperation)
        ...
        extrudes.addByDistanceExtent(sk_f2.profiles.item(0), adsk.core.ValueInput.createByReal(mm_to_cm(f_t)), adsk.fusion.FeatureOperations.JoinFeatureOperation)
    ...
    def notify(self, args):
        try:
            ...
            params = {
                ...
                'pitch': inputs.itemById('pitch').value * 10,
                'tooth_h': inputs.itemById('tooth_h').value * 10,
                'pld': inputs.itemById('pld').value * 10,
...
class AdvancedPulleyCreatedHandler(adsk.core.CommandCreatedEventHandler):
    def notify(self, args):
        ...
        # 1. 벨트 규격 그룹
        group1 = inputs.addGroupCommandInput('belt_grp', '1. 벨트 스펙 입력')
        ...
        group1.children.addValueInput('pitch', '피치 (Pitch, mm)', 'mm', adsk.core.ValueInput.createByReal(0.8)) # S8M 기본값
        group1.children.addValueInput('tooth_h', '이빨 높이 (mm)', 'mm', adsk.core.ValueInput.createByReal(0.305))
        group1.children.addValueInput('pld', '피치 라인 오프셋(PLD, mm)', 'mm', adsk.core.ValueInput.createByReal(0.0686))
        ...
        group2.children.addValueInput('clearance', '치형 공차 (Gap, mm)', 'mm', adsk.core.ValueInput.createByReal(0.01))
